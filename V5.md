...
- [ ] **Merge `ClientCloser` into `Client`**
  ...
  optional interface. CouchDB does not implement `ClientCloser`.

- [ ] **`Updater.Update()` should return the response string**
  CouchDB update functions return `[newDoc, response]`. The current
  `driver.Updater.Update()` signature only returns `(rev string, err error)`,
  discarding the response string from the JS function.

- [ ] **Allow empty `docID` in `Update()` for null-document updates**
  CouchDB supports `POST /{db}/_design/{ddoc}/_update/{func}` with no docID
  for update functions that create new documents. Kivik currently rejects
  empty `docID` at the client layer. Consider allowing it.
...
- [ ] **`Flusher` fallback should be a no-op**
  When a driver doesn't implement `driver.Flusher`, Kivik currently returns
  `StatusNotImplemented`. Since flushing is a hint, not a guarantee, the
  fallback should silently succeed (no-op) instead of erroring.
...
- [ ] **`execution_stats` for `_find`**
  CouchDB returns query execution statistics (total docs examined, keys
  examined, execution time) when `execution_stats: true` is set. Kivik
  doesn't currently surface this. Would require a new interface or extending
  `driver.Rows` to carry stats alongside results.
