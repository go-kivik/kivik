...
func (c *Config) GetSection(ctx context.Context, secName string) (map[string]string, error) {
	if sectioner, ok := c.Config.(driver.ConfigSection); ok {
		sec, err := sectioner.GetSection(ctx, secName)
		if errors.StatusCode(err) == http.StatusNotFound {
			err = nil
		}
		return sec, err
	}
	conf, err := c.GetAll(ctx)
	if err != nil {
		return nil, err
	}
...
func (c *Config) Get(ctx context.Context, secName, key string) (string, error) {
	if itemer, ok := c.Config.(driver.ConfigItem); ok {
		return itemer.Get(ctx, secName, key)
	}
	sec, err := c.GetSection(ctx, secName)
	if err != nil {
		return "", err
	}
...
func (c *Config) IsSet(ctx context.Context, secName, key string) bool {
	_, err := c.Get(ctx, secName, key)
	return err == nil
}

// GetString returns the requested value as a string.
func (c *Config) GetString(ctx context.Context, secName, key string) string {
	value, _ := c.Get(ctx, secName, key)
	return value
}

// GetInt returns the requested value as an int64.
func (c *Config) GetInt(ctx context.Context, secName, key string) int64 {
	value, _ := c.Get(ctx, secName, key)
...
func (c *Config) GetBool(ctx context.Context, secName, key string) bool {
	value, _ := c.Get(ctx, secName, key)
