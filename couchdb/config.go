...
func (c *client) SetConfigValue(ctx context.Context, node, section, key, value string) (string, error) {
	...
	err := c.DoJSON(ctx, http.MethodPut, configURL(node, section, key), opts, &old)
	return old, err
}

func (c *client) DeleteConfigKey(ctx context.Context, node, section, key string) (string, error) {
	var value string
	err := c.DoJSON(ctx, http.MethodDelete, configURL(node, section, key), nil, &value)
	return value, err
}
