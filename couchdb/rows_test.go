...
func TestRowsIterator(t *testing.T) {
	rows := newRows(t.Context(), io.NopCloser(strings.NewReader(input)))
	var count int
	for {
		row := &driver.Row{}
		err := rows.Next(row)
...
func TestMultiQueriesRowsIterator(t *testing.T) {
	rows := newMultiQueriesRows(t.Context(), io.NopCloser(strings.NewReader(multipleQueries)))
...
func TestRowsIteratorErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			rows := newRows(t.Context(), io.NopCloser(strings.NewReader(test.input)))
			for i := 0; i < 10; i++ {
...
func TestFindRowsIterator(t *testing.T) {
	rows := newFindRows(t.Context(), io.NopCloser(strings.NewReader(findInput))).(fullRows)
	var count int
	for {
		row := &driver.Row{}
		err := rows.Next(row)
