...
func (a *BasicAuth) Authenticate(c *Client) error {
	...
	req, err := c.NewRequest(MethodGet, "/_session", nil)
	if err != nil {
		return err
	}
	req.SetBasicAuth(a.Username, a.Password)
...
func (a *BasicAuth) Logout(c *Client) error {
	if c.Transport != a {
...
func (a *CookieAuth) Authenticate(c *Client) error {
	...
	return c.DoError(MethodPost, "/_session", &Options{JSON: a})
}
...
func (a *CookieAuth) Logout(c *Client) error {
	err := c.DoError(MethodDelete, "/_session", nil)
	if a.setJar {
		c.Jar = nil
	}
	return err
