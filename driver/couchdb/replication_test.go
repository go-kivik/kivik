...
func TestQualifyDSN(t *testing.T) {
	type dsnTest struct {
		Name      string
		DSN       string
		ClientDSN string
	...
	tests := []dsnTest{
		{
			Name:      "FullLocal",
			DSN:       "http://foo:bar@foo.com:9000/foo",
			ClientDSN: "http://foo:bar@foo.com:9000/",
			Expected:  "http://foo:bar@foo.com:9000/foo",
		},
		{
			Name:      "FullRemote",
			DSN:       "http://foo:bar@bar.com:9000/foo",
			ClientDSN: "http://foo:bar@foo.com:9000/",
			Expected:  "http://foo:bar@bar.com:9000/foo",
		},
		{
			Name:      "Local",
			DSN:       "foo",
		...
		},
		{
			...
			DSN:       "foo.com/foo",
			ClientDSN: "http://foo:bar@foo.com:9000/",
			Expected:  "http://foo:bar@foo.com:9000/foo.com/foo",
		},
	}
	for _, test := range tests {
		func(test dsnTest) {
			t.Run(test.Name, func(t *testing.T) {
				result := qualifyDSN(test.DSN, test.ClientDSN)
