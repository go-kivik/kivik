...
type DB struct {
	...
	PutAttachmentFunc    func(ctx context.Context, docID, rev, filename, contentType string, body io.Reader, options map[string]interface{}) (newRev string, err error)
	GetAttachmentFunc    func(ctx context.Context, docID, rev, filename string, options map[string]interface{}) (contentType string, md5sum driver.MD5sum, body io.ReadCloser, err error)
...
func (db *DB) PutAttachment(ctx context.Context, docID, rev, filename, cType string, body io.Reader, opts map[string]interface{}) (string, error) {
	return db.PutAttachmentFunc(ctx, docID, rev, filename, cType, body, opts)
}

// GetAttachment calls db.GetAttachmentFunc
func (db *DB) GetAttachment(ctx context.Context, docID, rev, filename string, opts map[string]interface{}) (string, driver.MD5sum, io.ReadCloser, error) {
