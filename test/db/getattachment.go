	...
	ctx.RunRW(func(ctx *kt.Context) {
		...
		ctx.Run("group", func(ctx *kt.Context) {
			ctx.RunAdmin(func(ctx *kt.Context) {
				ctx.Parallel()
				testGetAttachments(ctx, ctx.Admin, dbname, "foo.txt")
				testGetAttachments(ctx, ctx.Admin, dbname, "NotFound")
			})
			ctx.RunNoAuth(func(ctx *kt.Context) {
				ctx.Parallel()
				testGetAttachments(ctx, ctx.NoAuth, dbname, "foo.txt")
				testGetAttachments(ctx, ctx.NoAuth, dbname, "NotFound")
			})
		})
	})
}

func testGetAttachments(ctx *kt.Context, client *kivik.Client, dbname, filename string) {
	ctx.Run(filename, func(ctx *kt.Context) {
		...
		att, err := db.GetAttachment("foo", "", filename)
